...
void Suspend(void)
{
  ...
  uint32_t reset_timeout;   /* RESET 等待超时计数，防死锁 */
	/* suspend preparation */
	/* ... */
	...
	/*poll for RESET flag in ISTR，带超时防止枚举失败时死锁 */
	reset_timeout = 100000;
	while((_GetISTR()&ISTR_RESET) == 0)
	{
	    if (--reset_timeout == 0) break;   /* 超时放弃，不再挂起 */
	}
	if ((_GetISTR()&ISTR_RESET) == 0)
	{
	    /* 未等到 RESET（枚举异常）：恢复端点状态并直接返回，不进入 STOP 模式 */
